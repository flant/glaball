...
func FromChannelQuery(ch chan interface{}, opt *Options) (linq.Query, error) {
	...
	if f := opt.OrderBy; f != nil {
		if first = m.GetByPath(f[0]); first == nil {
	...
	query = query.Select(func(i interface{}) interface{} {
		...
		case Element:
			...
			key = reflectx.FieldByIndexesReadOnly(reflect.ValueOf(v.Struct), first.Index).Interface()
			if first.Name == "host" {
				key = v.Host.Project
			}
			return Result{
...
func OrderBy(groupBy, orderBy *reflectx.FieldInfo) func(i interface{}) interface{} {
	...
	if orderBy == nil || orderBy.Name == byLenFI.Name {
